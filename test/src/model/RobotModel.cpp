...
class MockSensorModel : public GenericSensorModel<uint8_t> {
public:
    MockSensorModel(const HardwareID &ID) : GenericSensorModel<uint8_t >(ID) {}
...
TEST_CASE( "Updating the model with null ID -> returns false", "[RobotModel]" ) {
    RobotModel model{};
    std::shared_ptr<SensorModel> sensorModel = std::make_shared<MockSensorModel>(MOCK_ID_1);
...
TEST_CASE( "Updating sensor correctly -> returns true, passes data to SensorModel object", "[RobotModel]" ) {
    ...
    REQUIRE(returnValue == true);
    REQUIRE(((MockSensorModel*)sensorModel.get())->getData() == MOCK_SENSOR_DATA);
}
